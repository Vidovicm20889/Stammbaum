...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      // Abmelde-Footer + List-Unsubscribe-Header (typ = Wochen-Digest-Spalte).
      const unsub = await unsubBundle(r.user_id, "email_woechentlicher_digest", s);
      const htmlBody = WRAP(inner + unsub.footer);

      const resp = await fetch("https://api.resend.com/emails", {
        method: "POST",
        headers: { "Authorization": `Bearer ${RESEND_API_KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({ from: MAIL_FROM, to, subject: T(s, "subject"), html: htmlBody, text: htmlZuText(htmlBody),
