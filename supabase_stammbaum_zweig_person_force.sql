-- =====================================================
-- ZWEIGBAUM AUS PERSON — Force-Option für gleichnamige Stammbäume
-- Ausführen in: Supabase -> SQL Editor. Idempotent. ERSETZT supabase_stammbaum_zweig_person.sql.
--
-- Hintergrund (Nutzerwunsch): Bisher bricht stammbaum_zweig_aus_person mit 'zweig_existiert'
-- ab, sobald im Verbund schon ein Baum desselben Nachnamens existiert. ZWEI verschiedene
-- Familien können aber denselben Nachnamen tragen (z. B. zwei unabhängige „Simić"). Daher
-- bekommt die RPC einen Parameter p_force: bei p_force=true wird die Existenzprüfung
-- übersprungen und ein WEITERER, eigenständiger Stammbaum (in eigener Familie, gleicher
-- Verbund) angelegt. Das Frontend warnt vorher ausdrücklich und holt eine Bestätigung ein.
--
-- Voraussetzungen wie supabase_stammbaum_zweig_person.sql (identitaet_id, merge_norm,
-- kann_familie_bearbeiten, verbund_id). Die alte 2-argige Variante wird entfernt, damit es
-- keine mehrdeutige Überladung gibt (PostgREST ruft die neue mit Default p_force=false auf).
-- =====================================================
...
CREATE OR REPLACE FUNCTION public.stammbaum_zweig_aus_person(
  p_wurzel uuid, p_name text, p_force boolean DEFAULT false)
...
BEGIN
  ...
  RETURN jsonb_build_object('ok', true, 'tree_id', v_tree, 'familie_id', v_new_fam,
                            'name', v_name, 'gespiegelt', v_n, 'force', p_force);
END $$;
GRANT EXECUTE ON FUNCTION public.stammbaum_zweig_aus_person(uuid, text, boolean) TO authenticated;

NOTIFY pgrst, 'reload schema';
SELECT 'Zweigbaum aus Person mit Force-Option: stammbaum_zweig_aus_person(p_wurzel, p_name, p_force)' AS status;
